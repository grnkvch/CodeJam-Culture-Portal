    ...
    <Layout>
      <Fragment>
        ...
        <div className="image-title">
          <img src={image} alt="Author's picture" />
        </div>
        ...
        <div id="timeline" className="title" style={{ textAlign: 'center' }}>
          <h1>{t(`arcTimeline`)}</h1>
        </div>
        <TimeLineComponent timelineData={arc.timelineData} />
        <div id="masterpiece" className="title" style={{ textAlign: 'center' }}>
          <h1>{t(`arcMasterpiece`)}</h1>
        </div>
        <TableComponent work={arc.work} />
        <div id="youtube" className="title" style={{ textAlign: 'center' }}>
          <h1>Youtube</h1>
        </div>
        <YoutubeComponent videoId={arc.videoId} t={t} />
        <div id="map" className="title" style={{ textAlign: 'center' }}>
          <h1>{t(`arcMap`)}</h1>
        </div>
        <MapComponent work={arc.work} />
        <div id="gallery" className="title" style={{ textAlign: 'center' }}>
          <h1>{t(`arcGallery`)}</h1>
        </div>
...
export const postQuery = graphql`
  query($lng: String!, $originalPath: String!) {
    ...
    javascriptFrontmatter(
      frontmatter: { path: { eq: $originalPath }, lng: { eq: $lng } }
    ) {
      frontmatter {
        path
        name
